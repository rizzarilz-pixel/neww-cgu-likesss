...
# ✅ نقطة النهاية الرئيسية - معالجة طلبات اللايك
@app.route('/like', methods=['GET', 'OPTIONS'])
def like_handler():
...
def home():
    return jsonify({
        ...
        "endpoints": {
            "/like": "GET - Send likes to a player (uid parameter required)",
            "/tokens": "GET - Get available tokens",
            "/": "GET - API status"
        }
    })

# ✅ معالج CORS لجميع الردود
@app.after_request
